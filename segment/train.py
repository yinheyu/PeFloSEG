...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        if opt.pruned:
            ...
            LOGGER.info("finetune the pruned model : %s" % weights)
        else:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            # Backward
            if opt.sr:
                loss.backward()  # mix precision train
            else:
                scaler.scale(loss).backward()  # mix precision train
            ...
            if opt.sr:
                ...
                for k, m in model.named_modules():
                    ...
                    if isinstance(m, nn.BatchNorm2d) and (k not in ignore_bn_list):
                        ...
                        m.bias.grad.data.add_(opt.sr * 10 * torch.sign(m.bias.data))  # L1, 这里不同
            # # ============================= sparsity training ========================== #
        ...
        if opt.sr:
            ...
            logger.tb.add_histogram('bn_bias/hist', bnb_weights.numpy(), epoch, bins='doane')
        # --------------------------------------------------------
        # Scheduler
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default=ROOT /'models/segment/yolov5s-segP2.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/custom.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
    parser.add_argument('--sr', type=float, default=0, help='是否开启稀疏训练, 不等于0是为 L1 normal sparse rate')  #amp需为Flase   no hlaf
    parser.add_argument('--pruned', type=float, default=0, help='是否对剪枝后模型进行finetune')
